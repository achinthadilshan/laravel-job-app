...
@section('content')
    ...
    <section class="px-4 py-8">
        <div class="max-w-screen-lg px-4 py-8 mx-auto bg-white rounded-lg md:px-8 dark:bg-slate-900">
            <h2 class="text-2xl font-semibold text-center text-slate-900 dark:text-white">Edit Job</h2>
            <p class="mb-6 text-center text-slate-600 dark:text-slate-400">Edit: {{ $job->title }}</p>
            <form action="{{ route('jobs.update', ['job' => $job]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <a href="{{ $job->image ? asset('storage/' . $job->image) : asset('images/no-image.png') }}"
                            target="_blank" class="mt-1 text-xs text-slate-600 dark:text-slate-400">{{ $job->image }}</a>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" value="{{ $job->type }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="6"
                            ...
                            placeholder="Write a job description here...">{{ $job->description }}</textarea>
                ...
                <div>
                    <button type="submit"
                        class="text-white bg-slate-700 hover:bg-slate-800 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-slate-800 dark:hover:bg-slate-700">Update</button>
                </div>
            </form>
        </div>
    </section>
